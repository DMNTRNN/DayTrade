...
def drawing(A):
    ...
    for t in range(20,W-P):
            rez[11,t]=min(np.log(max(A[t,4]*(1-(A[t,1]-A[t,2])/(2*(A[t,1]-A[t,2])-abs(A[t,3]-A[t,0])))/m1, 1/2.72**2)),2)
            rez[0,t]=1-1/(1+np.sum(poz[t-10:t])/max(np.sum(neg[t-P:t]),0.000000001)) #RSI
            rez[5,t]=1-1/(1+np.sum(poz1[t-10:t])/max(np.sum(neg1[t-P:t]),0.000000001)) #RSI-like feature
...
for name in coinlist:
    ...
    DATA=pd.DataFrame(Rez,columns=['open','high','low','close', 'volumeB','volumeQ','trades','takerVol','ma100','ma25','vwap','BTCopen'])
    DATA.dropna()
